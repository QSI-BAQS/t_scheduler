...
class TCultivator(TGenerator):
    ...
    def __init__(self, generator=None):  # RNG
        super().__init__(
            n_cycles=22, height=1, width=1, n_emitted=1, generator=generator
        )
        self.curr_stage = 0
        self.n_stages = 4
    ...
    def update(self, prob: bool = True):
        ...
        if self._curr_cycle == self.stages[self.curr_stage][self.STAGE_CYCLES]:
            ...
            if (
                prob
                and self._generator.random()
                < self.stages[self.curr_stage][self.STAGE_PROB]
            ):
                self.curr_stage += 1
